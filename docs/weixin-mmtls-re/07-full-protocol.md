...
**UA 扩展前字段观察**（3 样本差分，Finished 后）：
```
23 002e <4B 时间戳> 0b0088 <32B 数据> ...
30 0063 <4B 时间戳> 020080 <32B 数据>
0e 0024 <4B 时间戳> 760080 <数据>
```
- 首字节（0x23/0x30/0x0e）= 字段类型
- 2B 长度（0x2e=46/0x63=99/0x24=36）
- 4B 时间戳/随机
- `XX0080` 变长字段（0x8000 标志位）
- 尾部 ASCII `"554718"`（客户端标识候选，另见 `"UnifiedPCWindows 10 x86_64"` UA）
